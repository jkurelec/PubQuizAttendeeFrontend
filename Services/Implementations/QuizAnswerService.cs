...
namespace PubQuizAttendeeFrontend.Services.Implementations
{
    public class QuizAnswerService : IQuizAnswerService
    {
        ...
        public async Task<IEnumerable<QuizEditionResultBriefDto>> GetEditionResults(int editionId)
            => await _http.GetFromJsonAsync<IEnumerable<QuizEditionResultBriefDto>>($"{BasePath}?editionId={editionId}")
        ...
        {
            var response = await _http.PostAsJsonAsync($"{BasePath}/points", roundDto);
            return response.IsSuccessStatusCode
        ...
        public async Task<QuizAnswerDetailedDto?> UpdateTeamRoundPoints(QuizAnswerDetailedDto answerDto)
        {
            var response = await _http.PutAsJsonAsync($"{BasePath}/points/{answerDto.Id}", answerDto);
            return response.IsSuccessStatusCode
                ? await response.Content.ReadFromJsonAsync<QuizAnswerDetailedDto>()
                : null;
        }
